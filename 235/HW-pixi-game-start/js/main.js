...
function gameLoop(){
    ...
    //clamp to keep the ship on screen
    let w2 = ship.width/2;
	...
	for (let c of circles){
        ...
        if (rectsIntersect(c,b)&&c.isAlive){
            fireballSound.play();
...
function fireBullet(e){
    ...
    //offset so they don't clump
    let offset = -10;
    if(levelNum> 1){
      for( let i = 0; i<3;i++){
        let x = new Bullet(0xFFFFFF,ship.x + offset,ship.y);
        bullets.push(x);
        gameScene.addChild(x);
        offset+=10;//mod offset
    }
    }
    else{
